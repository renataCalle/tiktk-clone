...
const Detail = ({postDetails}: IProps) => {
  ...
  const handleLike = async (like: boolean) => {
    if(userProfile) {
      ...
      // open up new object, spread the previous state of the post, and then
      // update the post with the updated likes
      setPost({...post, likes: data.likes});
    }
  }

  const addComment = async (e: { preventDefault: () => void }) => {
    e.preventDefault();  //website doesn't reload when a comment is posted
    ...
    <>
      ...
      bg-white flex-wrap lg:flex-nowrap'>
        ...
        bg-no-repeat bg-cover bg-center'>
          <div className='opacity-80 absolute top-6 left-2 lg:left-6 flex
          gap-6 z-50'>
              ...
              <div>
                <Link href={`/profile/${post.postedBy._id}`}>
                  <div className='mt-3 flex flex-col gap-2'>
                    ...
                    text-primary'>{post.postedBy.userName} {` `}
                    <GoVerified className='text-blue-400 text-md'/>                
                    </p>
                    <p className='capitalize font-medium text-xs text-gray-500
                    md:block hidden'>{post.postedBy.userName}</p>
                  </div>
                </Link>
              </div>
            </div>
